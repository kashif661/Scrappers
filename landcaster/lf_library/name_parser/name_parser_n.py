...
def split_name(name, input_format="LFM"):
    ...
    if input_format == "LFM":
        # Logic for input format "LFM"
        suffix, component = get_suffix(name.split())
        result['suffix'] = suffix
        name_without_suffix = name.replace(suffix, '').strip()
        
        # Applying strip to the entire name after removing the suffix
        name_without_suffix = ' '.join(name_without_suffix.split())
        
        component = name_without_suffix.split(' ')
        
        if len(component) > 0:
            result['last_name'] = clean_text(component[0])

        if len(component) > 1:
            result['first_name'] = clean_text(component[1])
            if len(component) > 2:
                result['middle_name'] = clean_text(' '.join(component[2:]))
    ...
    elif input_format == "FML":
        # Logic for input format "FML"
        suffix, component = get_suffix(name.split())
        result['suffix'] = suffix
        name = name.replace(suffix, '')

        component = name.split(' ')
        component = list(filter(lambda item: item.strip(), component))
        result['first_name'] = clean_text(component[0])

        if len(component) > 1:
            result['middle_name'] = component[1]

        if len(component) > 2:
            result['last_name'] = clean_text(component[2])
        else:
...
if __name__ == '__main__':
    print(split_name('Mckinney Roscoe', "LFM"))
    # print(split_name('Zambelis, Aspasia'))
